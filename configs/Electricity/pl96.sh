#!/bin/bash
# Best config for Electricity pred_len=96
cd "$(dirname "$0")/../.."
export CUDA_VISIBLE_DEVICES=${1:-0}

for seed in 2021 2022 2023; do
  python -m gse_xlstm fit+test \
    --data ForecastingDataModule --data.dataset_name "Electricity" \
    --data.seq_len 768 --data.pred_len 96 --data.label_len 0 \
    --data.batch_size 16 --data.num_workers 2 --data.persistent_workers true \
    ...
    --trainer.logger.name "Elec_pl96_seed${seed}" \
    --trainer.logger.project gse-xlstm \
    --trainer.enable_progress_bar true \
    --model_checkpoint.dirpath "/tmp/gse_xlstm_ckpt/Elec_pl96_seed${seed}" \
    --model_checkpoint.save_top_k 1 --model_checkpoint.monitor val/loss
done

echo "=== Electricity pl=96 DONE (3 seeds) ==="
