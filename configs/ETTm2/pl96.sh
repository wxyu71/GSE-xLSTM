...
for seed in 2021 2022 2023; do
  python -m gse_xlstm fit+test \
    ...
    --trainer.max_epochs 40 --seed_everything $seed \
    --trainer.logger.name "ETTm2_pl96_seed${seed}" \
    --trainer.logger.project gse-xlstm \
    --trainer.enable_progress_bar true \
    --model_checkpoint.dirpath "/tmp/gse_xlstm_ckpt/ETTm2_pl96_seed${seed}" \
    --model_checkpoint.save_top_k 1 --model_checkpoint.monitor val/loss
done

echo "=== ETTm2 pl=96 DONE (3 seeds) ==="
